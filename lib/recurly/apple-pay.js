...
class ApplePay extends Emitter {
  ...
  get lineItems () {
    ...
    let lineItems = [].concat(this.config.lineItems);
    lineItems.push(this.total);
    return lineItems;
  }
  ...
  onPricingChange (price) {
    ...
    // TODO: Determine line item types

    // Plan
    this.config.lineItems.push({ type: '', label: this.planLabel, amount: price.now.plan });

    // Add-ons are repreented as a single line item
    if (!!this.config.pricing.items.addons.length) {
      this.config.lineItems.push({ type: '', label: 'Add-ons', amount: price.now.addons });
    }

    // Setup fee
    if (!!parseInt(price.now.setup_fee, 10)) {
      this.config.lineItems.push({ type: '', label: 'Setup fee', amount: price.now.setup_fee });
    }

    // Discount
    if (!!parseInt(price.now.discount, 10)) {
      this.config.lineItems.push({ type: '', label: 'Discount', amount: price.now.discount });
    }

    // Taxes
    if (!!parseInt(price.now.tax, 10)) {
      this.config.lineItems.push({ type: '', label: 'Taxes', amount: price.now.tax });
    }

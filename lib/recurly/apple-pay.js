...
class ApplePay extends Emitter {
  ...
  get finalTotalLineItem () {
    return Object.assign({}, this.total, { type: 'final' });
  }
  ...
  updatePriceFromPricing () {
    ...
    let taxAmount = pricing.price.taxes || pricing.price.tax;

    if (pricing.price.discount) lineItems.push(lineItem('Discount', pricing.price.discount));
    lineItems.push(lineItem('Subtotal', pricing.price.subtotal));
    if (taxAmount) lineItems.push(lineItem('Tax', taxAmount));
    if (pricing.price.giftCard) lineItems.push(lineItem('Gift card', pricing.price.giftCard));

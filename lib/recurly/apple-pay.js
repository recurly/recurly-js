...
class ApplePay extends Emitter {
  ...
  configure (options) {
    ...
    if ('total' in options) this.config.total = options.total;
    else return this.fatal = this.error('apple-pay-config-missing', { opt: 'total' });
  ...
  onValidate (event) {
    ...
    this.recurly.request('post', '/apple_pay/start', { validationURL }, (err, session) => {

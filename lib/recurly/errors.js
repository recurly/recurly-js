...
class ErrorDirectory {
   ...
   * @param {EventDispatcher} [reporter] EventDispatcher instance to report errors
   * @return {RecurlyError}
   * @throws {Error} if the requested error is not in the directory
   */
  get (code, context = {}, reporter) {
    if (!(code in this.ERROR_MAP)) {
      throw new Error(`invalid error: ${code}`);
    } else {
      return this.ERROR_MAP[code](context, reporter);
    }
  }
}
...
function recurlyErrorFactory (definition) {
  ...
  return function recurlyError (context = {}, reporter) {
    return new RecurlyError(code, message, context, reporter, help);
  }
}
 ...
 * @param {EventDispatcher} [reporter] EventDispatcher instance used to report an error
 * @param {String} [help] documentation reference
 */
class RecurlyError extends Error {
  constructor (code, message, context, reporter, help) {
    super();
    ...
    if (this.reporter) {
      ...
      console.error('TO BE DISPATCHED', this);

...
const debug = _debug('recurly:paypal:strategy:complete');
...
function setupTokenOptionsFromPricing (checkoutPricing) {
  ...
  const billingPlan = {
    // Plan name is sanitized to satisfy PayPal validations.
    // This allows alphanumerics and spaces, and replaces underscores and hyphens with spaces.
    name: plan.name.replace(/[_-]/g, ' ').replace(/[^0-9a-zA-Z\s]/g, ''),
    billing_cycles: [],
    one_time_charges
  };

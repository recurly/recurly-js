...
exports.attach = function (el) {
  ...
  function bind () {
    each(recurlyElements(), function (elem) {
      // 'zip' -> 'postal_code'
  ...
  function observe () {
    if (false &&MutationObserver) {
      debug('using MutationObserver');
      new MutationObserver(rebind).observe(el, { subtree: true, childList: true });
    } else {
      debug('using DOMSubtreeModified');
      events.bind(el, 'DOMSubtreeModified', rebind);
    }
  }

  function rebind () {
    if (dirty.apply(this, arguments)) {
      debug('rebinding');
      detach();
      bind();
    }
  ...
  function dirty (mutations) {
    ...
    if (mutations.length) {
      ...
      var newElems = [];
      each(recurlyElements(), function (elem) {
        newElems.push(dom.data(elem, 'recurly'));
      });
      dirty = newElems.join('-') !== object.keys(elems).join('-');
    }
  ...
  function recurlyElements () {
    return el.querySelectorAll('[data-recurly]');
  }
};

...
const debug = _debug('recurly:pricing:subscription-pricing');
...
export default class SubscriptionPricing extends Pricing {
  ...
  get hasAnyUsageAddons () {
    return this.items.plan?.addons?.some(a => a.add_on_type === 'usage');
  }

  get hasRampIntervals () {
    return Array.isArray(this.items.plan?.ramp_intervals) && this.items.plan.ramp_intervals.length > 0;
  }
  ...
  */
  giftcard (code, done) {
    ...
    return new PricingPromise(function (resolve, reject) {
      ...
      self.recurly.giftCard({ code }, function (err, gift_card) {

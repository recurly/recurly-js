...
export default class SubscriptionPricing extends Pricing {
  constructor (recurly, { id = uuid()} = {}) {
    ...
    this.id = id;
    this.debug = debug;
    this.recurly.report('pricing:create:subscription');
  }
  ...
  */
  ...
  bindReporting () {
    super.bindReporting();
    this.on('change:external', price => this.reportChangeEvent(price));
  }

  /**
   * Handles a price change report
   * @param  {[type]} price [description]
   * @return {[type]}       [description]
   */
  reportChangeEvent (price) {
    this.recurly.report('pricing:subscription:change', {
      price: {

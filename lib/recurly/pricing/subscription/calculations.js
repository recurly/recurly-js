...
export default class Calculations {
  ...
  get planPrice () {
    ...
    const basePrice = plan.price[this.items.currency];
    ...
    const price = {
      ...
      amount: basePrice.unit_amount * quantity
    };

    if (basePrice.price_segments) {
      price.price_segments = basePrice.price_segments;
    }
  ...
  plan () {
    ...
    if (base.price_segments) {
      this.price.base.plan.price_segments = base.price_segments;
    }
  ...
  discount () {
    ...
    if (coupon.discount.rate) {
      this.price.now.discount = roundRateDiscount(this.price.now.subtotal, coupon.discount.rate);
      this.price.next.discount = roundRateDiscount(this.price.next.subtotal, coupon.discount.rate);

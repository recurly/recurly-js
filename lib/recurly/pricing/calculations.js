...
function Calculations (pricing, done) {
  ...
  this.tax(function () {
    var planPrice = this.items.plan.price[this.items.currency];

    this.price.plan.unit = planPrice.unit_amount;
    this.price.plan.setup = planPrice.setup_fee;

    this.total();

    each(this.price.plan, decimal, this.price.plan);

...
const debug = _debug('recurly:pricing');
...
export class Pricing extends Emitter {
  ...
  guardTaxSignature (tax = {}) {
    ...
    if (typeof tax.amount !== 'object') {
      throw new Error(`Invalid 'amount'. Expected 'Object', got '${typeof tax.amount}'`);
    } else if (!isFinite(+tax.amount.now)) {
      throw new Error(`Invalid 'amount.now'. Expected 'Finite Numeric', got '${typeof tax.amount.now}'`);
    } else if (!isFinite(+tax.amount.next)) {
      throw new Error(`Invalid 'amount.next'. Expected 'Finite Numeric', got '${typeof tax.amount.next}'`);
    }
  }

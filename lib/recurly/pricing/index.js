...
export class Pricing extends Emitter {
  ...
  bindReporting () {
    ...
    const setGiftCard = giftCard => report('pricing:set:giftCard', { amount: giftCard.unit_amount });
    const unsetGiftCard = () => report('pricing:unset:giftCard');
    this.on('set.addon', addOn => report('pricing:set:addOn', pick(addOn, ['code', 'quantity'])));
    this.on('set.coupon', coupon => report('pricing:set:coupon', { code: coupon.code }));
    this.on('set.currency', code => report('pricing:set:currency', { code }));
    this.on('set.gift_card', setGiftCard);
    this.on('set.giftCard', setGiftCard);
    this.on('set.plan', plan => report('pricing:set:plan', { code: plan.code }));
    this.on('unset.coupon', () => report('pricing:unset:coupon'));

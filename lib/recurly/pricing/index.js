...
export class Pricing extends Emitter {
  ...
  bindDispatchedEvents () {
    ...
    const setGiftCard = giftCard => send({ name: 'pricing:set:giftCard', meta: { amount: giftCard.unit_amount } });
    const unsetGiftCard = () => send({ name: 'pricing:unset:giftCard' });
    this.on('set.addon', addOn => send({ name: 'pricing:set:addOn', meta: pick(addOn, ['code', 'quantity']) }));
    this.on('set.coupon', coupon => send({ name: 'pricing:set:coupon', meta: { code: coupon.code } }));
    this.on('set.currency', code => send({ name: 'pricing:set:currency', meta: { code } }));
    this.on('set.gift_card', setGiftCard);
    this.on('set.giftCard', setGiftCard);
    this.on('set.plan', plan => send({ name: 'pricing:set:plan', meta: { code: plan.code } }));
    this.on('unset.coupon', () => send({ name: 'pricing:unset:coupon' }));
    this.on('unset.gift_card', unsetGiftCard);
    this.on('unset.giftCard', unsetGiftCard);
    this.on('change:external', price => send({ name: 'pricing:change', meta: { price } }));
  }

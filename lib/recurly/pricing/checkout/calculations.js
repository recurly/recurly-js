...
export default class Calculations {
  ...
  discounts () {
    ...
    this.items.subscriptions.forEach(sub => promise.then(() => sub.coupon().reprice()))
  ...
  applyFreeTrialCoupon () {
    let promise = Promise.resolve();
    if (!this.hasValidSubscriptions) return promise;

    if (coupon.redemption_resource === 'subscription') {
      promise.then(() => mostValuableSubscriptionForFreeTrial().coupon(coupon).reprice());
    } else {
      this.validSubscriptions.forEach(sub => {
        promise.then(() => sub.coupon(coupon).reprice());
      });
    }

    // recompute subscriptions for the new free trials
    return promise.then(this.subscriptions);
  }

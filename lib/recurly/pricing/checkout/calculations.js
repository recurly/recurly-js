...
export default class Calculations {
  ...
  /**
   * TODO: make async to support async discounts
   */
  subtotal () {
    ...
    this.discount().then((resolve, reject) => {
  ...
  subscriptions () {
    ...
    this.validSubscriptions.forEach(subscription => {
  ...
  discount () {
    ...
    if (coupon.discount.rate) {
      ...
      if (coupon.redemption_resource === 'subscription') {
        ...
        this.validSubscriptions.forEach(sub => promise.then(() => sub.coupon(coupon).done()));
      }

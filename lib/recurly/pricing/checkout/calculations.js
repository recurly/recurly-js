...
export default class Calculations {
   ...
   *    - Whether coupon applies at account or subcription level (does this affect adjustments?)
   */
  discount () {
    const coupon = this.items.coupon;
    const convert = amt => parseFloat((Math.round(amt * 100) / 100).toFixed(6));

    this.price.now.discount = 0;
    this.price.next.discount = 0;

    // TODO: Adjust for plan-unbound coupons
    if (!coupon) return;
    if (coupon.discount.rate) {
      this.price.now.discount = convert(this.price.now.subtotal * coupon.discount.rate);
      this.price.next.discount = coupon.single_use ? 0 : convert(this.price.next.subtotal * coupon.discount.rate);

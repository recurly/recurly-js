...
export default class CheckoutPricing extends Pricing {
   ...
   * @param {String}  [adjustment.accountingCode] ???
   * @param {Boolean} [adjustment.taxExempt] whether this adjustment is tax exempt (default: false)
   * @param {String}  [adjustment.taxCode] (default: null)
   * @param {Number}  [adjustment.quantity] number of units (default: 1)
   * @param {String}  [adjustment.code] product code (default: null)
   * @public
   */
  adjustment (adjustment) {
    return new PricingPromise((resolve, reject) => {
      // TODO: Finalize adjustment validations
      if (typeof adjustment !== 'object') return this.error(errors('invalid-adjustment', { adjustment }), reject, 'adjustment');

      adjustment.amount = Number(adjustment.amount);
      if (!isFinite(adjustment.amount)) return this.error(errors('invalid-adjustment', { adjustment }), reject, 'adjustment');

      this.items.adjustment.push(adjustment);

      debug('set.adjustment');
      self.emit('set.adjustment', adjustment);
      resolve(adjustment);
    });
  }

...
export default class CheckoutPricing extends Pricing {
  constructor (...args) {
    super(...args);
    this.debug = debug;
    this.recurly.report('pricing:create:checkout');
  }
  ...
  bindReporting () {
    ...
    this.on('set.subscription', subscription => report('pricing:set:subscription'));
    this.on('change:external', price => report('pricing:change', {
      price: {

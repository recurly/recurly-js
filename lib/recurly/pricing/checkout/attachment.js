...
export default class Attachment extends Emitter {
      ...
      return subscription
        ...
        .then(() => {
          ...
          return Promise.all(subscriptionElems.addon.map(addonElem => {
            ...
            return subscription.addon(code, { quantity }).catch(e => this.pricing.error(e));
          }));
        })
        .then(() => {
          // Tax code
   ...
   *
   * - FIXME: Should there be output elements for each item in `price[when].items`?
   *
   */
  updateOutput (price) {
    const elems = this.elements;
    ...
    ['subscriptions', 'adjustments', 'discount', 'subtotal', 'tax', 'total'].forEach(value => {

  ...
  onInputChange (event) {
    ...
    Promise.all(Object.keys(subscriptionGroups).map(id => {
      ...
      let subscription = new SubscriptionPricing(this.recurly)
        .plan(dom.value(subscriptionElems.plan));
      ...
      // Tax code
      // TODO: This needs to be an attribute and not a separate element
      if (subscriptionElems.tax_code) {
        subscription = subscription.tax({ tax_code: dom.value(subscriptionElems.tax_code)});
      }

      return this.pricing = this.pricing.subscription(subscription.reprice());
    }))
    .then(() => {
      // Adjustments
      // TODO: Does this need the Promise.all structure?
      return Promise.all(elems.adjustment.map(adjustmentElem => {
        ...
        return this.pricing = this.pricing.adjustment({ code, amount, quantity, currency, taxCode, taxExempt });
      }));
    })
    .then(() => {
      // Currency
      if (!elems.currency) return;
      return this.pricing = this.pricing.currency(dom.value(elems.currency));
    })
    .then(() => {
      // Coupon
      if (!elems.coupon) return;
      return this.pricing = this.pricing
    ...
    .then(() => {
      ...
      if (elems.country || elems.postal_code) {
        return this.pricing = this.pricing.address({
    ...
    .then(() => {
      ...
      return this.pricing = this.pricing.tax({ vat_number: dom.value(elems.vat_number) });
    })
    .done(() => {

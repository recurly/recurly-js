...
export class ApplePay extends Emitter {
  ...
  /**
   * Loads the Apple Pay JS SDK to enable Apple Pay in non-Safari browsers.
   * Resolves immediately if ApplePaySession is already available.
   *
   * @return {Promise}
   * @private
   */
  _loadSDK () {
    ...
    debug('Loading Apple Pay SDK');
    return loadScriptPromise(APPLE_PAY_SDK_URL).catch(err => {
      debug('Apple Pay SDK failed to load', err);
...
function combinedInputs () {
  ...
  const fraud = this.recurly?.fraud?.params ? this.recurly.fraud.params(inputs) : {};

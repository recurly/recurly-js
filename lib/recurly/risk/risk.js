...
const debug = _debug('recurly:risk');
...
export class Risk {
   ...
   * @return {Promise}
   */
  static preflight ({ recurly, number, month, year, cvv, addressFields, gatewayCode, token }) {
    const data = {};
    ...
    return recurly.request.get({ route: '/risk/preflights', data })
      .then(({ preflights }) => {
        ...
        return ThreeDSecure.preflight({ recurly, number, month, year, cvv, preflights, addressFields, token });
      })
      .then(({ tokenType, risk }) => ({
  ...
  add (concern) {
    if (!(concern instanceof RiskConcern)) {
      throw new Error(`Invalid concern. Expected 'RiskConcern', got '${typeof concern}'`);
    }

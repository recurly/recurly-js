...
const debug = _debug('recurly:risk:three-d-secure:cybersource');

export default class CybersourceStrategy extends ThreeDSecureStrategy {
  static strategyName = 'cybersource';

  static preflight ({ recurly, number, month, year, gateway_code, token }) {
    ...
    let data = {
      gateway_type: CybersourceStrategy.strategyName,
      gateway_code,
    };
    ...
    return recurly.request.post({ route: '/risk/authentications', data })
      .then(({ jwt, deviceDataCollectionUrl }) => {
        ...
        return Promise.race([new Promise(resolve => {
          ...
          }, CybersourceStrategy.PREFLIGHT_TIMEOUT);
        })]);
      });
  }

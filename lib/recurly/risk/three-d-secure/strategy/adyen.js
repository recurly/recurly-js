...
export default class AdyenStrategy extends ThreeDSecureStrategy {
  ...
  get adyenFingerprintToken () {
    return this.actionToken.three_d_secure.params.authentication['threeds2.fingerprintToken'];
  }

  get adyenChallengeToken () {
    return this.actionToken.three_d_secure.params.authentication['threeds2.challengeToken'];
  }
  ...
  fingerprint () {
    ...
    const fingerprintService = adyenCheckout.create('threeDS2DeviceFingerprint', {
      ...
      onError: cause => this.threeDSecure.error('3ds-auth-error', { gateway: 'Adyen', cause }),
    });
  ...
  challenge () {
    ...
    const challengeService = adyenCheckout.create('threeDS2Challenge', {
      ...
      onComplete: results => this.emit('done', results),
      onError: cause => this.threeDSecure.error('3ds-vendor-auth-error', { gateway: 'Adyen', cause }),
      size: '05'
    });

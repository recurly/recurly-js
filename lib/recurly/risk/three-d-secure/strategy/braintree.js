...
const debug = _debug('recurly:risk:three-d-secure:braintree');
...
export default class BraintreeStrategy extends ThreeDSecureStrategy {
  ...
  static preflight ({ recurly, number, month, year, cvv, addressFields, token }) {
  ...
  /**
   * Merchant-defined custom fields used by the Braintree 3D Secure Rules manager.
   * @see {@link https://developer.paypal.com/braintree/docs/guides/3d-secure/rules-manager/javascript/v3/#custom-fields}
   */
  get customFields () {
    return this.threeDSecure.recurly.config.risk.threeDSecure.proactive.customFields;
  }
  ...
  attach (element) {
    ...
    this.whenReady(() => {
      ...
      const { braintree, braintreeClientToken, amount, nonce, bin, billingInfo, customFields } = this;
      ...
      if (customFields && Object.keys(customFields).length > 0) {
        verifyCardOptions.customFields = customFields;
      }

...
const PARENT_CREATE_TOKEN_TYPES = [
  'adyen_component_state'
];


const tokenizeOnBus = (recurly, inputs) => recurly.config.parent && !PARENT_CREATE_TOKEN_TYPES.includes(inputs.type);
const cardTokenization = (inputs) => !PARENT_CREATE_TOKEN_TYPES.includes(inputs.type)
...
function token (customerData, bus, done) {
  ...
  } else if (cardTokenization(inputs)) {

...
const isPreflightObject = (recurly) => typeof recurly.config.risk.threeDSecure.preflightDeviceDataCollector == 'object';
const billingInofoIdPresent = (recurly) => isPreflightObject(recurly) && !!recurly.config.risk.threeDSecure.preflightDeviceDataCollector.billingInfoId;
...
function token (customerData, bus, done) {
  ...
  if (cardTokenization(inputs)) {
    ...
    if ((number && month && year) || (cvv && billingInofoIdPresent(this))) {
      const addressFields = {
        first_name,
        last_name,
        address1,

...
import CREDIT_CARD_TYPES from './validate/credit-card-types.json';

const debug = _debug('recurly:validate');
...
export function cardType (number, partial = false) {
  ...
  // Elo overlaps some of the Discover BIN ranges
  if (types.length == 2 && types.includes('elo') && types.includes('discover')) {
    return 'elo';
  }

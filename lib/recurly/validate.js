 ...
 */
function buildRangeValue (start, length, terminator) {
  return parseInt(start.toString().substr(0, length).padEnd(length, terminator));
}
...
export function cardType (number, partial = false) {
  ...
  let firstDigits = cardNumber.substr(0, compareLength);
  if (partial) {
    firstDigits = firstDigits.padEnd(compareLength, '0');
  }
  const compareValue = parseInt(firstDigits);

  const types = Object.entries(creditCardTypes).filter(([type, groups]) => {
    ...
    return find(groups, ((group) => {
      if (!partial && !group.lengths.includes(cardNumber.length)) {
        return false;
      }

      const rangeFound = find(group.ranges, ([rangeBegin, rangeEnd]) => {
        const start = buildRangeValue(rangeBegin, compareLength, '0');
        const end = buildRangeValue(rangeEnd, compareLength, '9');
  ...
  return types.length == 1 && types[0][0] || 'unknown';
}

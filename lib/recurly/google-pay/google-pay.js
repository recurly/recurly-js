...
const debug = _debug('recurly:google-pay');
...
export class GooglePay extends Emitter {
  ...
  token (paymentData) {
    ...
    const fraud = this.recurly?.fraud?.params ? this.recurly.fraud.params(data) : [];
    ...
    return this.recurly.request.post({
      ...
      data: {
        ...
        ...data,
        ...(this.options.testCard && { test_card: this.options.testCard }),
      ...
      .then(token => this.runPreflightChecks(token))
      .then(token => {
  ...
  runPreflightChecks (token) {
    if (token.type !== 'credit_card') return Promise.resolve(token);

    return Risk.preflight({
      recurly: this.recurly,
      gatewayCode: this.config.gatewayCode,
      token,
    })
      .then(({ risk }) => {
        if (!risk || !risk.length) return token;

        debug('preflight data received', risk);
        return this.recurly.request.put({
          route: `/tokens/${token.id}`,
          data: { risk },
        });
      })
      .catch(err => {
        debug('preflight error', err);
        return token;
      });
  }

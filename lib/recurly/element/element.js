...
const debug = _debug('recurly:element');
...
export default class Element extends Emitter {
  ...
  get url () {
    ...
    const path = this.recurly.isCanaryOneThousandthSession ? '/field-canary.html' : '/field.html';

    return this.recurly.url(`${path}#config=${config}`);
  }

...
export default class Element extends Emitter {
  ...
  constructor ({ elements, ...options }) {
    ...
    this.on(`element:${this.id}:ready`, this.onReady.bind(this));
    this.on(`element:${this.id}:state:change`, this.onStateChange.bind(this));
    this.on(`element:${this.id}:submit`, this.onSubmit.bind(this));
   ...
   */
  attach (parent) {
    const parentEl = dom.element(parent);
    if (!parentEl) throw new Error(`Invalid parent. Excepted HTMLElement.`);
  ...
  destroy () {
    ...
    // we do this again in case the element is destroyed having been detached externally
    this.bus.remove(this.window);

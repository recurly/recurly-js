...
export class HostedField extends Emitter {
  ...
  injectOld () {
    const hidden = dom.createHiddenInput();
  ...
  inject () {
    ...
    this.target.children[0].insertAdjacentElement('afterbegin', dom.createHiddenInput());
    this.target.children[0].children[0].insertAdjacentHTML('afterend', hostedIframe);
    this.target.children[0].insertAdjacentElement('beforeend', dom.createHiddenInput());

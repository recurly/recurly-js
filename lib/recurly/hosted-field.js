...
export class HostedField extends Emitter {
  ...
  inject () {
    ...
    this.hiddenInputProxy = dom.createHiddenInput();
    this.hiddenInputProxy.addEventListener('focus', () => {
      this.focus();
    });
  ...
  // TODO: Move this out into a generic message factory
  message (name, body = {}) {
    global.parent.postMessage(new Message(`hostedField:${name}`, merge(true, body, { type: this.config.type })), '*');
  }
}

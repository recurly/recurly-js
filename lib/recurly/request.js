...
const debug = _debug('recurly:Request');
...
export class Request {
  ...
  put ({ route, data, done }) {
    return this.requestWithCallback({ method: 'put', route, data, done });
  }
  ...
  xhr ({ method, url, data }) {
    return new Promise((resolve, reject) => {
      ...
      const cancelAbort = () => this.recurly.off('destroy', abort);
      const bail = (...args) => {
        cancelAbort();
        reject(this.recurly.error(...args));
      };

      this.recurly.on('destroy', abort);
      ...
      req.open(method, url);
      req.timeout = timeout;
      req.ontimeout = () => bail('api-timeout');
      req.onerror = () => bail('api-error');
      req.onprogress = NOOP;
      req.onload = function () {
        let res;
        try {

...
export class Request {
   ...
   */
  get ({ route, data, done, cached, stringifyOptions }) {
    return this.requestWithCallback({ method: 'get', route, data, done, cached, stringifyOptions });
  }
   ...
   * @return {undefined} if `done` is passed in options -- the callback will be used instead
   */
  post ({ route, data, done, cached, stringifyOptions }) {
    return this.requestWithCallback({ method: 'post', route, data, done, cached, stringifyOptions });
  }
   ...
   * @return {undefined} if `done` is passed in options -- the callback will be used instead
   */
  requestWithCallback ({ method, route, data, done, stringifyOptions, cached = false }) {
    const request = this[cached ? 'cached' : 'request']({ method, route, data, stringifyOptions });
   ...
   * @return {Promise}
   */
  request ({ method, route, data = {}, stringifyOptions = {} }) {
    ...
    if (this.shouldUseXHR) {
      return this.xhr({ method, url, data, stringifyOptions });
    } else {
      return this.jsonp({ method, url, data, stringifyOptions });
    }
  }
   ...
   * @return {Promise}
   */
  xhr ({ method, url, data, stringifyOptions }) {
    const error = (...args) => this.recurly.error(...args);

    return new Promise((resolve, reject) => {
      let req = new XHR;
      const payload = qs.stringify(data, { encodeValuesOnly: true, ...stringifyOptions });
      const { timeout } = this;
   ...
   * @param {Object} [data]
   */
  jsonp ({ method, url, data, stringifyOptions }) {
    const { timeout, recurly } = this;

    if (method === 'post') data._method = method;
    url += `?${qs.stringify(data, { encodeValuesOnly: true, ...stringifyOptions })}`;

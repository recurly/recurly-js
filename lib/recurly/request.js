...
export class Request {
  ...
  xhr ({ method, url, data }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const complete = () => this.recurly.off('destroy', abort);
      ...
      req.open(method, url);
      req.timeout = timeout;
      req.ontimeout = () => complete(reject(error('api-timeout')));
      req.onerror = () => complete(reject(error('api-error')));
      req.onprogress = NOOP;
      req.onload = function () {
        let res;
        try {
          ...
          return complete(reject(error('api-error', {
        ...
        if (res && res.error) {
          complete(reject(error('api-error', res.error)));
        } else {

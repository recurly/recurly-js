...
Calculations.prototype.coupon = function () {
  ...
  if (coupon) {
    if (coupon.discount.rate) {
      this.price.now.subtotal -= this.price.now.subtotal * coupon.discount.rate;
      this.price.next.subtotal -= this.price.next.subtotal * coupon.discount.rate;
    } else {
      this.price.now.subtotal -= coupon.discount.amount_in_cents[this.items.currency];
      this.price.next.subtotal -= coupon.discount.amount_in_cents[this.items.currency];
    }
  }
};

...
Calculations.prototype.subtotal = function () {
  var subtotal = this.planPrice().unit_amount;
  ...
  if (this.items.address) {
    ...
    this.pricing.recurly.tax(this.items.address, function applyTax (err, taxes) {
      if (err) {
        ...
        self.price.now.tax = Math.ceil(self.price.now.tax);
        self.price.next.tax = Math.ceil(self.price.next.tax);
      }
      done.call(self);
    });
  } else done.call(this);
...
function decimal (prop) {
  this[prop] = Math.round(Math.max(this[prop], 0)).toFixed(2);
}

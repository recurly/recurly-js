...
import { Engage } from './recurly/engage';
...
import { publicMethods as validatePublicMethods } from './recurly/validate';

const debug = _debug('recurly');
...
export class Recurly extends Emitter {
  ...
  tax = tax;
  token = token;
  validate = validatePublicMethods;
  Venmo = Venmo;
  ...
  constructor (options) {
    ...
    this.engage = new Engage({ recurly: this });
  ...
  configure (options) {
    ...
    if ('engage' in options) {
      deepAssign(this.config.engage, options.engage);
    }

...
import FraudDataCollector from './recurly/fraud-data-collector';
...
export class Recurly extends Emitter {
  ...
  configure (options) {
    ...
    if ('fraudDataCollector' in options) {
      this.config.fraudDataCollector = options.fraudDataCollector;
    }

    if ('litleSessionId' in options) {
      this.config.litleSessionId = options.litleSessionId;
    }
    ...
    if (this.config.fraudDataCollector) FraudDataCollector.addFields.call(this);
  };

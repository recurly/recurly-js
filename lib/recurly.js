...
export class Recurly extends Emitter {
  ...
  configure (options) {
    ...
    if (options.api) {
      ...
      this.config.origin = (new URL(options.api)).origin;
    }
  ...
  parent () {
    ...
    if (this.hostedFields) {
      ...
      if (this.readyState > 1 && !this.hostedFields.integrityCheck()) reset = true;
      // If we are switching hosted field locations, reset
      const selectors = map(this.config.fields, f => f.selector).join('');
      const currentSelectors = map(this.hostedFields.config.fields, f => f.selector).join('');
      if (selectors !== currentSelectors) reset = true;
    }

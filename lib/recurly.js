...
export class Recurly extends Emitter {
  ...
  configure (options) {
    ...
    if ('risk' in options) {
      normalizeRisk(this.config.risk, options.risk);
...
/**
 * Normalizes the risk configuration
 * 
 * @param  {Object} config
 * @param  {Object} options
 */
function normalizeRisk (config, options) {
  const threeDSecureConfig = config.threeDSecure || {};
  const threeDSecureOptions = options.threeDSecure || {};

  if (typeof threeDSecureConfig.preflightDeviceDataCollector === 'boolean' &&
      typeof threeDSecureOptions.preflightDeviceDataCollector === 'object') {
    threeDSecureConfig.preflightDeviceDataCollector = { billingInfoId: 'test-billing-info-id' };
  } else if (typeof threeDSecureConfig.preflightDeviceDataCollector === 'object' &&
             typeof threeDSecureOptions.preflightDeviceDataCollector === 'boolean') {
    threeDSecureConfig.preflightDeviceDataCollector = true;
  }

  config.threeDSecure = threeDSecureConfig;
}

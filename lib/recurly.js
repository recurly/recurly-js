...
export class Recurly extends Emitter {
   ...
   *
   * Chunks are made of data[chunkBy] in size chunkSize. If any request errors,
   * the entire pipeline errors
   *
   * @param {String} method
   * @param {String} route
   * @param {Object} data
   * @param {Array} data[chunkBy] must be an Array
   * @param {String} chunkBy
   * @param {Number} chunkSize
   * @param {Function} done
   * @private
   */

  pipedRequest (method, route, data, chunkBy, chunkSize, done) {
    let results = [];
    const part = (err, res) => {
      if (err) {
        done(err)
      ...
      results.push(res);
      if (results.length === dataChunks.length) done(null, results);
    };

    const dataChunks = chunk(data[chunkBy], chunkSize).map(chunkPiece => {
      return Object.assign({}, data, { [chunkBy]: chuckPiece });
    });

    dataChunks.forEach(dataChunk => this.request(method, route, dataChunk, part));
  }

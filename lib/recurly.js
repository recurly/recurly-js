...
export class Recurly extends Emitter {
  ...
  configure (options) {
    ...
    if (type(options.fields) === 'object' || this.hostedFields) {
      this.parent(options);
    } else {
  ...
  parent (options) {
    ...
    if (this.readyState > 0) {
      this.bus.send('hostedFields:configure', fieldOptions);
      return;
    }
    this.hostedFields = new this.HostedFields(fieldOptions);
    this.bus = new Bus;
    this.bus.add(this);
    ...
    this.hostedFields.once('hostedFields:ready', payload => {
      this.bus.send('hostedFields:ready', payload);
      this.readyState = 2;
    });
    this.readyState = 1;
  };
...
Recurly.prototype.HostedFields = HostedFields;
Recurly.prototype.Bus = Bus;
Recurly.prototype.Message = Message;

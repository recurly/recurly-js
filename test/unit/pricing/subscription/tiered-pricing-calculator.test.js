...
describe('Recurly.Pricing.Subscription.TieredPricingCalendar', function () {
  ...
  describe('with tiered addons', () => {
    it('should apply the tiered cost to the addon price', function (done) {
      const tieredAddon = TIERED_PLAN.addons.find((addon) => addon.code == 'tiered');
    ...
    it('should apply the stairstep cost to the addon price', function (done) {
      const stairstepAddon = TIERED_PLAN.addons.find((addon) => addon.code == 'stairstep');
    ...
    it('should apply the volume cost to the addon price', function (done) {
      const volumeAddon = TIERED_PLAN.addons.find((addon) => addon.code == 'volume');
